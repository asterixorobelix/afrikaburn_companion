...
class ThemeCampRepositoryImpl(
    ...
    override fun observeThemeCampsByEvent(eventId: String): Flow<List<ThemeCamp>> {
        return themeCampQueries.selectThemeCampsByEvent(eventId).asFlow().mapToList().map { list ->
    ...
    override suspend fun saveThemeCamp(themeCamp: ThemeCamp) = withContext(Dispatchers.IO) {
        themeCampQueries.insertThemeCamp(
            ...
            contactInfo = themeCamp.contactInfo,
            activities = themeCamp.activities.joinToString(","),
            amenities = themeCamp.amenities.joinToString(","),
            qrCode = themeCamp.qrCode,
            photoUrl = themeCamp.photoUrl,
            isHidden = if (themeCamp.isHidden) 1L else 0L,
            unlockTimestamp = themeCamp.unlockTimestamp,
...
/**
 * Placeholder for database theme camp entity
 * This would be generated by SQLDelight from the schema
 */
