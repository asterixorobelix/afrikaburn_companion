...
class EventRepositoryImpl(
    ...
    override fun observeAllEvents(): Flow<List<Event>> {
        return eventQueries.selectAllEvents().asFlow().mapToList().map { list ->
            list.map { it.toEvent() }
        }
    }
    
    override fun observeCurrentEvent(): Flow<Event?> {
        return eventQueries.selectCurrentEvent().asFlow().mapToOneOrNull().map { 
            it?.toEvent() 
        }
    }
...
private fun DatabaseEvent.toEvent(): Event {
    return Event(
        id = id,
        year = year.toInt(),
        startDate = kotlinx.datetime.LocalDate.fromEpochDays(startDate.toInt()),
        endDate = kotlinx.datetime.LocalDate.fromEpochDays(endDate.toInt()),
...
/**
 * Placeholder for database event entity
